...
module Jekyll
  ...
  class AliasGenerator < Generator
    ...
    def generate_aliases(destination_path, aliases)
      ...
      alias_paths.flatten.each do |alias_path|
        ...
        File.open(File.join(fs_path_to_dir, alias_file), 'w') do |file|
          file.write(alias_template(destination_path, @site.baseurl))
        end
    ...
    def alias_template(destination_path, baseurl)
      <<-EOF
      <!DOCTYPE html>
      <html>
      <head>
      <link rel="canonical" href="#{baseurl}#{destination_path}"/>
      <meta http-equiv="content-type" content="text/html; charset=utf-8" />
      <meta http-equiv="refresh" content="0;url=#{baseurl}#{destination_path}" />
      </head>
      </html>
      EOF
    end
